...
unsigned long KefVolon = 0x1FE40BF;
unsigned long CxcEject = 0xD2D;
...
void loop() {
  ...
  if (irrecv.decode(&results)) {
    ...
    if (results.value == SonyTVon) {
      beep();
      Serial.println("Kef on");
      for (int i = 0; i < 10; i++) {
    ...
    if (results.value == CxcEject || results.value == 0x52d) {
      beep();
      Serial.println("CXC on");
      for (int i = 0; i < 10; i++) {
        irsend.sendNEC(0x1FE40BF, 32);
        delay(40);
      }
